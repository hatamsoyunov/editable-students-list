...
const StudentsItem: React.FC<Props> = ({ student }) => {
    ...
    const handleRemove = (id: string): void => {
        if (window.confirm('Вы уверены, что хотите удалить данного студента?')) {
    ...
    const renderDisplay = ({ id, name, birthDay, progress }: IStudent) => {
        return (
            <TableRow hover className={listItemClass}>
                ...
                <TableCell>
                    {progressList.map((progressItem: IProgressList) => (progressItem.id === progress ? progressItem.label : null))}
                </TableCell>
                <TableCell>
